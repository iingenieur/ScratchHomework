...
5개 발판을 올라가 깃발에 도달! 거북이가 발판 위를 순찰하며 방해.
거북이에 닿으면 하트 -1, 처음 위치로 리셋. 하트 0 = 게임오버.
"""

from common import (
    BB, AssetManager, GY, gen_beep, uid,
...
def main():
    ...
    BR = {k: uid() for k in ["시작", "스테이지1", "게임오버", "클리어", "리셋"]}
    ...
    # ════════════════════════════════════════════════════════════════════════
    # PLATFORMS (5개 - 지그재그 계단, 벽돌 3개씩 이어붙인 크기)
    # ════════════════════════════════════════════════════════════════════════
    # Layout: 계단식 (low-left → high-right)
    # (name, x, y, sprite_file)
    # 간격을 벌려서 점프로 넘어가야 함
    PLAT_POSITIONS = [
        ("Plat1", -130, -70, "plat_4.png"),
        ("Plat2",   30, -10, "plat_5.png"),
        ("Plat3",  180,  50, "plat_4.png"),
    ]
    ...
    m = BB()
    mf = m.flag()
    m.chain([mf, m.goto(MARIO_START_X, MARIO_START_Y), m.set_size(80), m.costume("걷기1"), m.hide()])
    ...
    # Platform landing checks (5개)
    def plat_check(pname):
        tp = m.touching(pname)
    ...
    plat_checks = [plat_check(n) for n, _, _, _ in PLAT_POSITIONS]

    # ── Movement ──
    kr  = m.key_pressed("right arrow")
    mvr = [m.change_x(5), m.point_dir(90)]
    m.chain(mvr)
    ifr = m.if_then(kr, mvr[0])

    kl  = m.key_pressed("left arrow")
    mvl = [m.change_x(-5), m.point_dir(-90)]
    m.chain(mvl)
    ...
    cj  = m.op_and(kj, cnj)
    jb  = [m.set_var("속도Y", V["속도Y"], 14), m.set_var("점프중", V["점프중"], 1), m.play_sound("Jump")]
    m.chain(jb)
    ...
    # ── Turtle collision: 하트 -1, 처음으로 리셋 ──
    TURTLE_NAMES = ["Turtle1", "Turtle2"]

    def turtle_hit(tname):
        tt = m.touching(tname)
        hit = [
        ...
        m.chain(hit)
        return m.if_then(tt, hit[0])

    turtle_hits = [turtle_hit(tn) for tn in TURTLE_NAMES]
    ...
    mario_sprite = {
        ...
        "x": MARIO_START_X, "y": MARIO_START_Y, "size": 80, "direction": 90,
    ...
    # Plat3(x=180, y=50, plat_4=128px) 오른쪽 끝에 깃발
    # 오른쪽 끝 = 180 + 64 - 10 = 234, 블록 위 = 50 + 20 = 70 (높이 넉넉히)
    fl.chain([flh, fl.goto(230, 130), fl.show()])
    ...
    flag_sprite = {
        ...
        "x": 230, "y": 130, "size": 200, "direction": 90,
        "draggable": False, "rotationStyle": "don't rotate",
    }
    ...
    # ════════════════════════════════════════════════════════════════════════
    # Turtle1: 바닥 왼쪽 끝 ↔ 오른쪽 끝
    # Turtle2: Plat2(x=30,y=-10) 위에서 순찰 (y=-10+16=6)
    # Turtle1: 바닥 - 오른쪽 끝에서 시작, 왼쪽을 보고 걸어옴
    # Turtle2: Plat2(x=30,y=-10) 위, 블록 위에 딱 올라온 느낌 (y=-10+18=8)
    # 쿠파 스프라이트가 기본 왼쪽을 바라봄 → Scratch에서:
    #   direction 90 (not flipped) = 원본 = 왼쪽 바라봄
    #   direction -90 (flipped) = 반전 = 오른쪽 바라봄
    TURTLE_PATROLS = [
        ("Turtle1", 220, -220, GY, 3.0),       # 바닥: 오른쪽에서 시작→왼쪽으로
        ("Turtle2", -40, 100,   14, 1.5),       # Plat2 위 (블록 위로 올림)
    ]

    def make_turtle(name, x1, x2, y, speed):
        t = BB()
        tf = t.flag()
        t.chain([tf, t.hide()])
        ...
        ti = [t.goto(x1, y), t.set_size(60), t.point_dir(start_face), t.show()]
        # 첫 이동
        ...
        return {
            ...
            "costumes": [am.reg_png("koopa", "koopa/koopa_walk_1.png")],
            "sounds": [], "volume": 100, "layerOrder": 4, "visible": False,
            "x": x1, "y": y, "size": 60, "direction": 90,
            "draggable": False, "rotationStyle": "left-right",
        }

    turtles = [make_turtle(*tp) for tp in TURTLE_PATROLS]
